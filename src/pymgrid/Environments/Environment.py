...
class Environment(gym.Env):
    ...
    def __init__(self, env_config, seed=42):
        ...
        self.observation_space = Box(
            low=-0.1, high=np.float("inf"), shape=(self.Ns,), dtype=np.float
        )
        ...
        if mg.architecture["grid"] == 1:
            if mg.grid.status == 1:
                control_dict["grid_import"] = max(
                    0,
                    action[2]
                    * min(action[3] * mg.grid.power_import, mg.grid.power_import),
                )
                control_dict["grid_export"] = max(
                    0,
                    (1 - action[2])
                    * min(action[3] * mg.grid.power_export, mg.grid.power_export),
                )
